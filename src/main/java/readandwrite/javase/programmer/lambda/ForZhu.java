...
public class ForZhu {
    public static void main(String[] args) {
        ...
        students.stream()
                .collect(Collectors.collectingAndThen(Collectors.toCollection(() -> new TreeSet<>(Comparator.comparing(f -> f.getName()))), ArrayList::new))
                .forEach(System.out::println);

        ArrayList<Student> collect2 = students.stream()
                .collect(Collectors.collectingAndThen(Collectors.toCollection(() -> new TreeSet<>(Comparator.comparing(f -> f.getName()))), ArrayList::new));

        // 求交集
        ArrayList<Student> collect3 = (ArrayList<Student>) collect2.stream().filter(item -> students.stream().map(e -> e.getName()).collect(Collectors.toList()).contains(item.getName())).collect(Collectors.toList());
